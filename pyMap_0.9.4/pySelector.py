...
class Tile_Selector:
    ...
    def load_tilesheet(self, image_name, trans = False):
        ...
        if temp_sheet_rect.width%self.tile_width or temp_sheet_rect.height%self.tile_height:
            ...
            for col in xrange(0, (self.tilesheet_rect.width/self.tile_width)):
                self.tiles.append([])
                for row in xrange(0, (self.tilesheet_rect.height/self.tile_height)):
    ...
    def clear_selected(self, pos):
        if self.selected_tiles:
            ...
            for col in xrange(self.tile_view_x[0], self.tile_view_x[1]):
                if not check_tiles:
                    break
                for row in xrange(self.tile_view_y[0], self.tile_view_y[1]):
                    if not check_tiles:
                        break
