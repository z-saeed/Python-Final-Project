...
class Canvas:
    ...
    def check_adjacent_cells(self):
        ...
        for item in self.selected_cells:
            ...
            col_list.append(col)
            row_list.append(row)
        for i in xrange(min(col_list), max(col_list)+1, 1):
            for j in xrange(min(row_list),max(row_list)+1, 1):
    ...
    def make_object(self, name):
        ...
        info = []
        i = 0
        for col in xrange(img_ref_start[0], img_ref_end[0]+1, 1):
            j = 0
            for row in xrange(img_ref_start[1], img_ref_end[1]+1, 1):
    ...
    def draw(self, surface):
        ...
        selection = []
        for layer in xrange(0, len(self.grid_ref), 1):
            if layer not in self.show_layer_list:
                continue
            else:
                for col in xrange(self.tile_view_x[0], self.tile_view_x[1], 1):
                        for row in xrange(self.tile_view_y[0], self.tile_view_y[1], 1):
    ...
    def select_area_rect(self):
        ...
        if self.select_point2:
            ...
            if not self.selected_cells:
                self.selected_cells = []
            deselect = []
            for col in xrange(tl.x, br.x+1, 1):
                pygame.event.pump()
                for row in xrange(tl.y, br.y+1, 1):
    ...
    def select_cells_from_tileset(self):
        ...
        if not self.tile_selector.add_to_selected:
                self.clear_selected()
        for col in xrange(len(self.grid_ref[0])):
            pygame.event.pump()
            for row in xrange(len(self.grid_ref[0][0])):
                if (col,row) in self.selected_cells:
                        continue
    ...
    def snap_view(self, pos):
        ...
        if self.view_rect.collidepoint(pos):
            ...
            if dist_to_centerx != 0:
                ...
                for i in xrange(0, abs(dist_to_centerx), 1):
    ...
    def insert_col(self):
        ...
        for layer in xrange(0, self.max_layer, 1):
            new_col = []
    ...
    def remove_col(self):
        ...
        if self.grid_ref[0]:
            ...
            self.clear_selected()
            if cols > 1:
                for layer in xrange(self.max_layer-1,-1,-1):
    ...
    def clear_selected(self):
        ...
        if self.selected_cells and not self.select_point1:
            for i in xrange(0, len(self.selected_cells), 1):
        ...
        if self.selected_cells and self.tile_selector.selected_tiles:
            ...
            if num_tiles_selected > 1:
                if not self.fill_ordered:#Left-Shift will toggle this properties Bool Value
                    shuffle_seq = []
                    for i in xrange(len(self.selected_cells)):
    ...
    def export_to_img(self):
        ...
        for layer in xrange(self.max_layer-1,-1,-1):
            for col in xrange(0, cells_w, 1):
                for row in xrange(0, cells_h, 1):
    ...
    def add_layer(self, tile_size = None, map_size = None):
        ...
        for col in xrange(map_w):
            grid.append([])
            x_offset = col*tile_w
            for row in xrange(map_h):
                y_offset = row*tile_h
    ...
    def show_info(self, surface):
        ...
        if self.status_msg:
            for i in xrange(0,len(self.status_msg),1):
    ...
    def save(self):
        ...
        cell_info = []
        for layer in xrange(len(self.grid_ref)):
            for col in xrange(len(self.grid_ref[layer])):
                for row in xrange(len(self.grid_ref[layer][col])):
                    if self.grid_ref[layer][col][row].img_ref_rect:
    ...
    def load(self, folder, screen = None, caller = None):
        ...
        next_line = map_file.readline().strip('\n')
        flagged_tiles = []
        for i in xrange(0, grid_layers-1, 1):
